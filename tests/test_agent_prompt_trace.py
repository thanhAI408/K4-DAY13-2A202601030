...
def test_agent_links_request_context_to_trace_metadata(monkeypatch) -> None:
    monkeypatch.setenv("LANGFUSE_PUBLIC_KEY", "test-public-key")
    monkeypatch.setenv("LANGFUSE_SECRET_KEY", "test-secret-key")
    client = RecordingLangfuseClient()
    monkeypatch.setattr(agent_module, "get_langfuse_client", lambda: client)
    ...
    trace_metadata = client.trace_updates[-1]["metadata"]
    generation_metadata = client.generation_updates[-1]["metadata"]
    assert trace_metadata["correlation_id"] == "req-1234abcd"
    assert trace_metadata["env"] == "test"
    assert generation_metadata["correlation_id"] == "req-1234abcd"
    assert generation_metadata["env"] == "test"
